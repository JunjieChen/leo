...
extern int cmp_mtime();
 ...
 int rev_xstrcoll_df_mtime (V a, V b) { do { _Bool a_is_dir = is_directory ((struct fileinfo const *) a); _Bool b_is_dir = is_directory ((struct fileinfo const *) b); if (a_is_dir && !b_is_dir) return -1; if (!a_is_dir && b_is_dir) return 1; } while (0); return cmp_mtime (b, a, xstrcoll); }